...
- Rift Sapper: beat the third campaign map on Normal or Hard
...
| Chill Shot | On hit: -30% enemy speed for 6 s; stacks multiplicatively per tower |
...
Normal targets ~75% bot win rate; Hard targets ~50%. Multipliers are tunable at runtime via the automated tuning pipeline.
...
10 achievements tracked locally via `AchievementManager`, persisted to `user://achievements.cfg`. Forwarded to Steam when available.
...
Current strategy set (11 strategies):
