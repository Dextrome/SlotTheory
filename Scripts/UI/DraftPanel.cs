...
public partial class DraftPanel : CanvasLayer
{
    ...
    private DraftOption? _pendingPremium;
    ...
    public bool IsAwaitingPremiumTarget => _pendingPremium != null;
    ...
    public string PlacementHint
    {
        ...
        if (_pendingPremium != null)
        {
            var pDef = Core.PremiumCardRegistry.TryGet(_pendingPremium.Id);
            string name = pDef?.Name ?? _pendingPremium.Id;
            return $"Tap a tower to apply {name}\nTowers already at {Core.Balance.MaxPremiumModSlots} mod slots are ineligible.";
        }
    ...
    public bool IsSlotValidTarget(int i)
    {
        ...
        if (IsAwaitingPremiumTarget && _pendingPremium?.Id == Core.PremiumCardRegistry.ExpandedChassisId)
            return slots[i].Tower != null && slots[i].Tower!.MaxModifiers < Core.Balance.MaxPremiumModSlots;
        return false;
    }
    ...
    public void SelectSlot(int slotIndex)
    {
        ...
        else if (_pendingPremium != null)    OnPremiumSlotPicked(slotIndex);
    }
    ...
    private void BuildCardRow(List<DraftOption> options)
    {
        ...
        // Adaptive sizing: always recompute when 6+ cards are shown, or on tablet/narrow viewports.
        if (cardCount > 5 || MobileOptimization.IsTablet() || layoutW < 1240f)
        {
        ...
        for (int i = 0; i < options.Count; i++)
        {
            ...
            bool isSuperRare  = isPremium && Core.PremiumCardRegistry.TryGet(opt.Id)?.Rarity == Core.PremiumRarity.SuperRare;
            AnimateCardReveal(btn, front, cardBack, accent, i, punchTargets.iconNode, punchTargets.titleNode,
                isFoil: i == _foilCardIndex, isPremium: isPremium, isPremiumSuperRare: isSuperRare);
        }
    }
    ...
    private void OnCardPressed(DraftOption opt, Button sourceButton)
    {
        ...
        if (opt.Type == DraftOptionType.Tower)
            _pendingTower = opt;
        else if (opt.Type == DraftOptionType.Premium && Core.PremiumCardRegistry.RequiresTarget(opt.Id))
            _pendingPremium = opt;
        else if (opt.Type == DraftOptionType.Modifier)
            _pendingModifier = opt;
        else if (opt.Type == DraftOptionType.Premium)
        {
            // Non-targeting premium: apply immediately, no slot selection needed.
        ...
        GetTree().CreateTimer(0.06f).Timeout += () =>
        {
            ...
            if (IsAwaitingSlot || IsAwaitingTower || IsAwaitingPremiumTarget) return;
            var tw = CreateTween();
            tw.SetParallel(true);
    ...
    private void OnPremiumSlotPicked(int slotIndex)
    {
        if (_pendingPremium == null) return;
        if (!IsSlotValidTarget(slotIndex)) return;
        ulong nowMs = Time.GetTicksMsec();
        if (_previewModifierSlot == slotIndex)
        {
            if (_previewSetAtMs > 0 && (nowMs - _previewSetAtMs) < 120)
    ...
    private void AnimateCardReveal(Button btn, Control front, Control cardBack, Color accent, int index, Control? iconNode, Control? titleNode, bool isFoil, bool isPremium = false, bool isPremiumSuperRare = false)
    {
        ...
        btn.GetTree().CreateTimer(delay).Timeout += () =>
        {
            ...
            // Premium cards: shimmer on reveal. Parent button clips children so effects stay within card bounds.
            if (isPremium)
            {
                btn.ClipContents = true;
                var sz = btn.CustomMinimumSize;
                var premiumDelay = CardEntranceSeconds * 0.55f;
                btn.GetTree().CreateTimer(premiumDelay).Timeout += () =>
                {
                    if (!GodotObject.IsInstanceValid(btn) || !GodotObject.IsInstanceValid(front)) return;
                    PlayPremiumRevealShimmer(front, sz, accent, isPremiumSuperRare);
                    if (isPremiumSuperRare) SoundManager.Instance?.Play("card_shing");
                    btn.GetTree().CreateTimer(0.7f).Timeout += () =>
                    {
                        if (GodotObject.IsInstanceValid(btn))
                            StartPremiumIdleSheen(btn, sz, accent, isPremiumSuperRare);
                    };
                };
            }
        };
    ...
    /// <summary>
    /// Triple-band shimmer sweep on premium card reveal. All children added to `front` which is
    /// inside btn (ClipContents=true), so nothing bleeds outside the card.
    /// </summary>
    private static void PlayPremiumRevealShimmer(Control front, Vector2 cardSize, Color accent, bool isSuperRare)
    {
        static void Sweep(Control parent, Vector2 sz, Color color, float width, float peakAlpha, float duration, float extraDelay = 0f)
        {
            var rect = new ColorRect
            {
                Color = new Color(color.R, color.G, color.B, 0f),
                Position = new Vector2(-width - 10f, 0f),
                Size = new Vector2(width, sz.Y),
                MouseFilter = Control.MouseFilterEnum.Ignore,
            };
            parent.AddChild(rect);
            var tw = rect.CreateTween();
            if (extraDelay > 0f) tw.TweenInterval(extraDelay);
            tw.SetParallel(true);
            tw.TweenProperty(rect, "color:a", peakAlpha, 0.055f);
            tw.TweenProperty(rect, "position:x", sz.X + 10f, duration)
                .SetTrans(Tween.TransitionType.Sine).SetEase(Tween.EaseType.InOut);
            tw.Chain().TweenProperty(rect, "color:a", 0f, 0.06f);
            tw.TweenCallback(Callable.From(() => { if (GodotObject.IsInstanceValid(rect)) rect.QueueFree(); }));
        }

        float dur = isSuperRare ? 0.38f : 0.44f;
        // Outer soft band (accent-tinted)
        Sweep(front, cardSize, accent,        width: 55f, peakAlpha: isSuperRare ? 0.28f : 0.18f, duration: dur);
        // Mid band (white)
        Sweep(front, cardSize, Colors.White,  width: 22f, peakAlpha: isSuperRare ? 0.50f : 0.36f, duration: dur);
        // Razor core (pure white)
        Sweep(front, cardSize, Colors.White,  width:  7f, peakAlpha: isSuperRare ? 0.92f : 0.72f, duration: dur);

        if (isSuperRare)
        {
            // Second pass delayed -- double glint
            float d2 = 0.28f;
            Sweep(front, cardSize, accent,        width: 40f, peakAlpha: 0.22f, duration: d2, extraDelay: 0.18f);
            Sweep(front, cardSize, Colors.White,  width: 10f, peakAlpha: 0.65f, duration: d2, extraDelay: 0.18f);

            // Full-card flash
            var flash = new ColorRect { Color = new Color(1f, 1f, 1f, 0f), MouseFilter = Control.MouseFilterEnum.Ignore };
            flash.SetAnchorsPreset(Control.LayoutPreset.FullRect);
            front.AddChild(flash);
            var ftw = flash.CreateTween();
            ftw.TweenInterval(0.18f);
            ftw.TweenProperty(flash, "color:a", 0.22f, 0.04f);
            ftw.TweenProperty(flash, "color:a", 0f, 0.30f).SetTrans(Tween.TransitionType.Expo).SetEase(Tween.EaseType.Out);
            ftw.TweenCallback(Callable.From(() => { if (GodotObject.IsInstanceValid(flash)) flash.QueueFree(); }));
        }

        // Sparkle dots
        int sparkCount = isSuperRare ? 8 : 5;
        for (int k = 0; k < sparkCount; k++)
        {
            float sx = (float)GD.RandRange(10, cardSize.X - 10);
            float sy = (float)GD.RandRange(8,  cardSize.Y - 8);
            float sd = (float)GD.RandRange(0.04, 0.14);
            float sz2 = (float)GD.RandRange(2.5, isSuperRare ? 5.5 : 4.0);
            var dot = new ColorRect
            {
                Color = new Color(1f, 1f, 1f, 0f),
                Position = new Vector2(sx - sz2 * 0.5f, sy - sz2 * 0.5f),
                Size = new Vector2(sz2, sz2),
                MouseFilter = Control.MouseFilterEnum.Ignore,
            };
            front.AddChild(dot);
            var dtw = dot.CreateTween();
            dtw.TweenInterval(sd);
            dtw.TweenProperty(dot, "color:a", isSuperRare ? 0.95f : 0.80f, 0.06f);
            dtw.TweenProperty(dot, "color:a", 0f, 0.18f).SetTrans(Tween.TransitionType.Expo).SetEase(Tween.EaseType.Out);
            dtw.TweenCallback(Callable.From(() => { if (GodotObject.IsInstanceValid(dot)) dot.QueueFree(); }));
        }
    }

    /// <summary>Slow looping sheen sweep that keeps the card visually alive after reveal.</summary>
    private static void StartPremiumIdleSheen(Control parent, Vector2 cardSize, Color accent, bool isSuperRare)
    {
        ...
        var sheen = new ColorRect
        {
            Color = new Color(1f, 1f, 1f, 0f),
            Position = new Vector2(-width - 10f, 0f),
            Size = new Vector2(width, cardSize.Y),
            MouseFilter = Control.MouseFilterEnum.Ignore,
        };
        ...
        doLoop = () =>
        {
            ...
            tw.TweenProperty(sheen, "position:x", cardSize.X + 10f, 0.52f)
                .SetTrans(Tween.TransitionType.Sine).SetEase(Tween.EaseType.InOut);
        ...
        parent.GetTree().CreateTimer(0.3f).Timeout += doLoop;
    }
    ...
    private static void ApplyCardStyle(Button btn, DraftOption opt)
    {
        ...
        static StyleBoxFlat Box(Color bg, Color border, int borderWidth = 2, int cornerRadius = 8)
        {
            var b = new StyleBoxFlat();
            b.BgColor = bg;
            b.BorderColor = border;
            b.BorderWidthTop = borderWidth;
            b.BorderWidthBottom = borderWidth;
            b.BorderWidthLeft = borderWidth;
            b.BorderWidthRight = borderWidth;
            b.CornerRadiusTopLeft = cornerRadius;
            b.CornerRadiusTopRight = cornerRadius;
            b.CornerRadiusBottomLeft = cornerRadius;
            b.CornerRadiusBottomRight = cornerRadius;
            return b;
        }
        ...
        if (isPremium)
        {
            bool isSuperRare = Core.PremiumCardRegistry.TryGet(opt.Id)?.Rarity == Core.PremiumRarity.SuperRare;
            Color bg = isSuperRare
                ? new Color(0.07f, 0.06f, 0.02f, 0.97f)
                : new Color(0.04f, 0.06f, 0.11f, 0.97f);

            static StyleBoxFlat PremBox(Color bg, Color border, Color shadow, int shadowSize, int cornerRadius = 9)
            {
                var b = new StyleBoxFlat();
                b.BgColor = bg;
                b.BorderColor = border;
                b.BorderWidthTop = b.BorderWidthBottom = b.BorderWidthLeft = b.BorderWidthRight = 2;
                b.CornerRadiusTopLeft = b.CornerRadiusTopRight = b.CornerRadiusBottomLeft = b.CornerRadiusBottomRight = cornerRadius;
                b.ShadowColor = shadow;
                b.ShadowSize = shadowSize;
                b.ShadowOffset = Vector2.Zero;
                return b;
            }

            Color shadowNormal = new Color(accent.R, accent.G, accent.B, isSuperRare ? 0.55f : 0.38f);
            Color shadowHover  = new Color(accent.R, accent.G, accent.B, isSuperRare ? 0.85f : 0.62f);
            Color dimBorder    = new Color(accent.R, accent.G, accent.B, 0.70f);

            btn.AddThemeStyleboxOverride("normal",  PremBox(bg, dimBorder, shadowNormal, isSuperRare ? 10 : 7));
            btn.AddThemeStyleboxOverride("hover",   PremBox(bg, accent,    shadowHover,  isSuperRare ? 18 : 14));
            btn.AddThemeStyleboxOverride("pressed", PremBox(bg, accent,    shadowHover,  isSuperRare ? 14 : 10));
            btn.AddThemeStyleboxOverride("focus",   PremBox(bg, accent,    shadowHover,  isSuperRare ? 14 : 10));
        }
        else
        {
    ...
    private static Color GetOptionAccent(DraftOption opt) => opt.Type switch
    {
        DraftOptionType.Modifier => ModifierVisuals.GetAccent(opt.Id),
        DraftOptionType.Premium  => GetPremiumAccent(opt.Id),
        _                        => GetTowerAccent(opt.Id),
    };

    private static Color GetPremiumAccent(string cardId)
    {
        var def = Core.PremiumCardRegistry.TryGet(cardId);
        return def?.Rarity == Core.PremiumRarity.SuperRare
            ? new Color(1.00f, 0.82f, 0.22f)   // gold
    ...
    private static (Control? iconNode, Control? titleNode) BuildPremiumCard(
        VBoxContainer root, string cardId, int titleSize, int tagSize, int bodySize, float iconSize)
    {
        var def = Core.PremiumCardRegistry.TryGet(cardId);
        bool isSuperRare = def?.Rarity == Core.PremiumRarity.SuperRare;
        var accent = GetPremiumAccent(cardId);
        ...
        var iconHolder = new ColorRect
        {
            Color = new Color(accent.R, accent.G, accent.B, 0.22f),
            CustomMinimumSize = new Vector2(iconSize, iconSize),
            Size = new Vector2(iconSize, iconSize),
            MouseFilter = Control.MouseFilterEnum.Ignore,
        };
        ...
        glyphLbl.SetAnchorsPreset(Control.LayoutPreset.FullRect);
        UITheme.ApplyFont(glyphLbl, semiBold: true, size: Mathf.RoundToInt(iconSize * 0.70f));
        glyphLbl.Modulate = new Color(accent.R, accent.G, accent.B, 0.95f);
        iconHolder.AddChild(glyphLbl);

        var titleLbl = new Label
        {
            Text = def?.Name ?? cardId,
            AutowrapMode = TextServer.AutowrapMode.Off,
        };
        UITheme.ApplyFont(titleLbl, semiBold: true, size: titleSize);
        titleLbl.Modulate = new Color(accent.R, accent.G, accent.B, 1f);
        top.AddChild(titleLbl);

        string rarityText = isSuperRare ? "SUPER RARE" : "RARE";
        var rarityLbl = new Label { Text = rarityText, AutowrapMode = TextServer.AutowrapMode.Off };
        UITheme.ApplyFont(rarityLbl, semiBold: true, size: tagSize);
        rarityLbl.Modulate = new Color(accent.R, accent.G, accent.B, 0.88f);
        root.AddChild(rarityLbl);

        var typeLbl = new Label { Text = "PROTOCOL", AutowrapMode = TextServer.AutowrapMode.Off };
        UITheme.ApplyFont(typeLbl, semiBold: true, size: Mathf.Max(10, tagSize - 1));
        typeLbl.Modulate = new Color(0.86f, 0.90f, 1.00f, 0.55f);
        root.AddChild(typeLbl);
        ...
        UITheme.ApplyFont(descLbl, size: bodySize);
        descLbl.Modulate = new Color(0.95f, 0.95f, 1.00f, 0.96f);
        root.AddChild(descLbl);

        return (iconHolder, titleLbl);
    }
    ...
    private static string TowerRole(string towerId) => towerId switch
    {
        ...
        "accordion_engine" => "Compression pulse hits all enemies in range and squeezes their spacing. Physically repositions enemies into tighter packs.",
        _ => "Generalist tower.",
    };
