...
public partial class HudPanel : CanvasLayer
{
	...
	private VBoxContainer _premiumTray = null!;
	private PanelContainer _premiumTooltip = null!;
	private Label _premiumTooltipLabel = null!;
	...
	public override void _Ready()
	{
		...
		// Premium card tray -- left side, below top bar
		_premiumTray = new VBoxContainer
		{
			LayoutMode     = 1,
			AnchorLeft     = 0f, AnchorRight  = 0f,
			AnchorTop      = 0f, AnchorBottom = 0f,
			OffsetLeft     = 6f,
			OffsetTop      = TopBarHeight + 4f,
			OffsetRight    = 168f,
			OffsetBottom   = TopBarHeight + 4f,
			MouseFilter    = Control.MouseFilterEnum.Stop,
		};
		_premiumTray.AddThemeConstantOverride("separation", 3);
		AddChild(_premiumTray);

		_premiumTooltip = new PanelContainer
		{
			LayoutMode   = 1,
			AnchorLeft   = 0f, AnchorRight  = 0f,
			AnchorTop    = 0f, AnchorBottom = 0f,
			OffsetLeft   = 0f, OffsetTop    = 0f,
			OffsetRight  = 200f, OffsetBottom = 60f,
			Visible      = false,
			ZIndex       = 20,
			MouseFilter  = Control.MouseFilterEnum.Ignore,
		};
		ApplyPremiumTooltipStyle(_premiumTooltip);
		_premiumTooltipLabel = new Label
		{
			AutowrapMode = TextServer.AutowrapMode.WordSmart,
			CustomMinimumSize = new Vector2(180f, 0f),
			MouseFilter = Control.MouseFilterEnum.Ignore,
		};
		UITheme.ApplyFont(_premiumTooltipLabel, semiBold: false, size: 12);
		_premiumTooltipLabel.AddThemeColorOverride("font_color", new Color(0.88f, 0.92f, 1.00f));
		_premiumTooltip.AddChild(_premiumTooltipLabel);
		AddChild(_premiumTooltip);
	...
	public void ResetSpeed()
	{
		...
		RefreshPremiumCards(System.Array.Empty<string>());
	}
	...
	public void RefreshPremiumCards(System.Collections.Generic.IReadOnlyList<string> cardIds)
	{
		...
		{
			var def = Core.PremiumCardRegistry.TryGet(id);
			if (def == null) continue;

			int count = counts[id];
			bool superRare = def.Rarity == Core.PremiumRarity.SuperRare;
			var accent = superRare
			...
			var capturedDef = def;
			chip.MouseEntered += () => ShowPremiumTooltip(chip, capturedDef);
	...
	private void ShowPremiumTooltip(Control anchor, Core.PremiumCardDef def)
	{
		...
		_premiumTooltip.OffsetTop    = pos.Position.Y;
	...
	private static void ApplyPremiumChipStyle(PanelContainer chip, Color accent)
	{
		...
		{
			BgColor           = new Color(0.06f, 0.10f, 0.16f, 0.88f),
			BorderColor       = new Color(accent.R, accent.G, accent.B, 0.50f),
			BorderWidthLeft   = 1, BorderWidthRight  = 1,
			BorderWidthTop    = 1, BorderWidthBottom = 1,
			CornerRadiusTopLeft    = 3, CornerRadiusTopRight    = 3,
			CornerRadiusBottomLeft = 3, CornerRadiusBottomRight = 3,
			ContentMarginLeft = 5f, ContentMarginRight  = 6f,
			ContentMarginTop  = 2f, ContentMarginBottom = 2f,
		};
		chip.AddThemeStyleboxOverride("panel", sb);
	}

	private static void ApplyPremiumTooltipStyle(PanelContainer tooltip)
	{
		var sb = new StyleBoxFlat
		{
			BgColor           = new Color(0.04f, 0.08f, 0.14f, 0.96f),
			BorderColor       = new Color(0.60f, 0.70f, 0.85f, 0.55f),
			BorderWidthLeft   = 1, BorderWidthRight  = 1,
			BorderWidthTop    = 1, BorderWidthBottom = 1,
		...
		tooltip.AddThemeStyleboxOverride("panel", sb);
	}
