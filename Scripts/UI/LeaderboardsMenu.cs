...
public partial class LeaderboardsMenu : Node
{
    ...
    private readonly Dictionary<string, bool> _rowExpansionOverrides = new();
    ...
    public override void _Ready()
    {
        ...
        frame.CustomMinimumSize = new Vector2(940f, 520f);
        ...
        var bodyPanel = new PanelContainer
        {
            ...
            CustomMinimumSize = new Vector2(940f, 450f),
        };
        ...
        _localButton  = MakeButton("Local",  140, 36, 20, OnLocalMode);
        _globalButton = MakeButton("Global", 140, 36, 20, OnGlobalMode);
        ...
        _mapOption = new OptionButton { CustomMinimumSize = new Vector2(360, 36) };
        ...
        _difficultyOption = new OptionButton { CustomMinimumSize = new Vector2(220, 36) };
        ...
        {
            CustomMinimumSize = new Vector2(900f, 340f),
            SizeFlagsHorizontal = Control.SizeFlags.ExpandFill,
            SizeFlagsVertical = Control.SizeFlags.ExpandFill,
        };
        ...
        pageRow.AddThemeConstantOverride("separation", 6);
        ...
        _pageLabel.AddThemeFontSizeOverride("font_size", 15);
        ...
        var back    = MakeButton("Back",    180, 38, 20, OnBack);
    ...
    private PanelContainer BuildEntryCard(LeaderboardEntryView row)
    {
        ...
        bool isTopPlacement = row.Rank <= 3;
        ...
        panel.CustomMinimumSize = new Vector2(880f, expanded ? 56f : 36f);
        panel.AddThemeStyleboxOverride("panel", UITheme.MakePanel(
            ...
            padH: 10,
            padV: expanded ? 2 : 1));
        UITheme.AddTopAccent(panel, new Color(0.70f, 0.92f, 1.00f, isTopPlacement ? 0.30f : 0.18f));

        var cardCol = new VBoxContainer();
        cardCol.AddThemeConstantOverride("separation", expanded ? 0 : 1);
        panel.AddChild(cardCol);

        var headerRow = new HBoxContainer();
        headerRow.AddThemeConstantOverride("separation", 6);
        cardCol.AddChild(headerRow);

        var header = new Label
        {
            HorizontalAlignment = HorizontalAlignment.Left,
            VerticalAlignment = VerticalAlignment.Center,
            Text = BuildRowSummaryText(row, expanded),
            ClipText = true,
            SizeFlagsHorizontal = expanded
                ? Control.SizeFlags.ExpandFill
                : Control.SizeFlags.ShrinkBegin,
            SizeFlagsVertical = Control.SizeFlags.ShrinkCenter,
        };
        UITheme.ApplyFont(header, semiBold: true, size: expanded ? 15 : 14);
        header.Modulate = isTopPlacement
            ? new Color(0.96f, 0.98f, 1.00f)
            : new Color(0.90f, 0.95f, 1.00f);
        if (!expanded)
            header.CustomMinimumSize = new Vector2(340f, 0f);
        headerRow.AddChild(header);

        if (!expanded)
        {
            var collapsedNameWrap = new HBoxContainer
            {
                SizeFlagsHorizontal = Control.SizeFlags.ExpandFill,
                SizeFlagsVertical = Control.SizeFlags.ShrinkCenter,
                Alignment = BoxContainer.AlignmentMode.Begin,
            };
            collapsedNameWrap.AddThemeConstantOverride("separation", 0);
            headerRow.AddChild(collapsedNameWrap);
            collapsedNameWrap.AddChild(new Control { CustomMinimumSize = new Vector2(6f, 0f) });

            var collapsedName = new RichTextLabel
            {
                BbcodeEnabled = true,
                ScrollActive = false,
                FitContent = true,
            ...
            collapsedName.AddThemeFontOverride("normal_font", UITheme.SemiBold);
            collapsedName.AddThemeFontSizeOverride("normal_font_size", 14);
            collapsedName.AddThemeConstantOverride("line_separation", 0);
            collapsedName.AppendText(BuildGradientBbCode(
                Truncate(buildStyle.Name, 30),
                buildStyle.StartColor,
                buildStyle.EndColor));
            collapsedNameWrap.AddChild(collapsedName);
        ...
        {
            var topBuildNameHeader = new Label
            {
                Text = "Build Name",
                HorizontalAlignment = HorizontalAlignment.Center,
            ...
            UITheme.ApplyFont(topBuildNameHeader, semiBold: true, size: 14);
            topBuildNameHeader.Modulate = new Color(0.70f, 0.84f, 0.98f, 0.82f);
            headerRow.AddChild(topBuildNameHeader);
        }

        var expandButton = MakeExpandButton(expanded, () =>
        {
            ToggleRowExpanded(row);
            RenderCurrentPage();
        });
        if (expanded)
            expandButton.CustomMinimumSize = new Vector2(30f, 18f);
        ...
        cardCol.AddChild(contentRow);

        var leftCol = new VBoxContainer();
        leftCol.AddThemeConstantOverride("separation", 2);
        ...
        var buildNameCenter = new CenterContainer
        {
            CustomMinimumSize = new Vector2(320f, 0f),
            SizeFlagsHorizontal = Control.SizeFlags.ExpandFill,
            SizeFlagsVertical = Control.SizeFlags.ShrinkBegin,
        };
        rightCol.AddChild(buildNameCenter);
        ...
        buildName.AddThemeFontSizeOverride("normal_font_size", 16);
        buildName.AddThemeConstantOverride("line_separation", 0);
        buildName.AppendText(BuildGradientBbCode(
            Truncate(buildStyle.Name, 30),
            buildStyle.StartColor,
            buildStyle.EndColor));
        buildNameCenter.AddChild(buildName);

        leftCol.AddChild(BuildLoadoutStrip(row.Build, compact: false));

        // Keep build-name column horizontally aligned with the top header, which sits before the expand button.
        contentRow.AddChild(new Control { CustomMinimumSize = new Vector2(30f, 0f) });

        // Give expanded entries a couple of bottom pixels so loadout icons stay inside the border.
        cardCol.AddChild(new Control { CustomMinimumSize = new Vector2(0f, 2f) });

        return panel;
    }

    private static string BuildRowSummaryText(LeaderboardEntryView row, bool expanded)
    {
        string baseSummary = $"#{row.Rank}  {Truncate(row.Name, 18)}  |  {row.Score:N0}";
        if (expanded)
            return $"{baseSummary}  |  W {row.WaveReached}/{Balance.TotalWaves}  |  L {row.LivesRemaining}  |  {FormatTime(row.TimeSeconds)}";
        return $"{baseSummary}  |  W {row.WaveReached}/{Balance.TotalWaves}  |  L {row.LivesRemaining}";
    }

    private bool IsRowExpanded(LeaderboardEntryView row)
    {
    ...
    {
        string key = BuildRowExpansionKey(row);
        _rowExpansionOverrides[key] = !IsRowExpanded(row);
        SoundManager.Instance?.Play("ui_select");
    }

    private static string BuildRowExpansionKey(LeaderboardEntryView row)
        => $"{row.Rank}|{row.Name}|{row.Score}|{row.WaveReached}|{row.LivesRemaining}|{row.TimeSeconds}|{row.IsLocal}";

    private static Button MakeExpandButton(bool expanded, System.Action callback)
    {
        var btn = new Button
        {
            Text = expanded ? "\u25B2" : "\u25BC",
            TooltipText = expanded ? "Collapse details" : "Expand details",
            CustomMinimumSize = new Vector2(30f, 20f),
            SizeFlagsHorizontal = Control.SizeFlags.ShrinkEnd,
            SizeFlagsVertical = Control.SizeFlags.ShrinkCenter,
        };
        ...
        btn.AddThemeStyleboxOverride("normal",  MakeExpandButtonStyle(
            bg: new Color(0.01f, 0.04f, 0.08f, 0.96f),
            border: new Color(0.06f, 0.30f, 0.33f, 0.95f),
            borderWidth: 1));
        btn.AddThemeStyleboxOverride("hover",   MakeExpandButtonStyle(
            bg: new Color(0.03f, 0.10f, 0.14f, 0.98f),
            border: new Color(0.14f, 0.90f, 0.96f, 0.98f),
            borderWidth: 2));
        btn.AddThemeStyleboxOverride("pressed", MakeExpandButtonStyle(
            bg: new Color(0.01f, 0.06f, 0.09f, 0.98f),
            border: new Color(0.10f, 0.64f, 0.70f, 0.96f),
            borderWidth: 2));
        btn.AddThemeStyleboxOverride("focus",   MakeExpandButtonStyle(
            bg: new Color(0.03f, 0.10f, 0.14f, 0.98f),
            border: new Color(0.14f, 0.90f, 0.96f, 0.98f),
            borderWidth: 2));
    ...
    private static StyleBoxFlat MakeExpandButtonStyle(Color bg, Color border, int borderWidth)
    {
    ...
    private Control BuildLoadoutStrip(RunBuildSnapshot build, bool compact = false)
    {
        int rowSeparation = compact ? 3 : 6;
        int slotSeparation = compact ? 1 : 2;
        int slotWidth = compact ? 30 : 46;
        int towerSize = compact ? 20 : 30;
        int modSeparation = compact ? 2 : 1;
        int modsLeftPad = compact ? 1 : 3;
        int modSize = compact ? 7 : 10;
        ...
        for (int i = 0; i < Balance.SlotCount; i++)
        {
            ...
            slotBox.CustomMinimumSize = new Vector2(slotWidth, 0f);
            ...
            if (slot.ModifierIds.Length == 0)
            {
                var emptyMod = new ColorRect
                {
                    ...
                    CustomMinimumSize = new Vector2(modSize, modSize),
                };
            ...
            else
            {
                foreach (var mod in slot.ModifierIds.Take(Balance.MaxModifiersPerTower))
                {
                    var modIcon = new ModifierIcon
                    {
                        ...
                        CustomMinimumSize = new Vector2(modSize, modSize),
                        Size = new Vector2(modSize, modSize),
                    };
                    if (compact)
                        modIcon.ClipContents = true;
