"""
map_compare.py - path length + avg tower coverage comparison across handcrafted maps.
Uses the same scoring logic as MapGenerator.ScoreCell (340px range, 30px sample spacing, bends x5).
...
def analyse_map(name, pts, slots):
    ...
    covered = sum(1 for (p, _) in samples if any(math.dist(p, s) <= RANGE for s in slots))
...
def load_maps():
    ...
    maps = {}
    order = []
    for m in sorted(data["maps"], key=lambda row: row.get("displayOrder", 999)):
